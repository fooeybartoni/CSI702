...
/* These functions are inlined in code below
...
double V(double x)
{
  ...
  //made into a define double PI=3.14159265;
...
double calckinetic(int nprt,int ndim,double mass,double *veloc)
{
  ...
  for(i=0; i<nprt; i++) {
    ...
    temp = square(magnitude(ndim,&veloc[i*ndim]));
    printf("temp is %f \n", temp);
    kin += 1.0/2.0*mass*temp;
  }
  return kin;
}
