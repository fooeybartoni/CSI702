...
inline void dist(int ndim,double *r1,double *r2,double *dr)
{
...
double V(double x)
{
  ...
  double vv=pow((sin(MIN(x,PI/2.0))),2.0);
  return vv;
}
...
double magnitude(int ndim,double *v1) 
{
  ...
  int    k;
  double d=0.0;
  for(k=0; k<ndim; k++) d+=pow((v1[k]),2.0);
  d=sqrt(d);
  return d;
}
  ...
  for(i=0; i<nprt; i++) {
    for(j=0; j<nprt; j++) {
      ...
      for(k=0; k<ndim; k++) {
         ...
         force[i*ndim+k] += -rij[k]*2.0*sin(MIN(d,PI/2.0))*cos(MIN(d,PI/2.0))/d;
      }
    }
  }
}
...
double calcpotent(int nprt,int ndim,double *coord)
{
  ...
  for(i=0; i<nprt; i++) {
    for(j=0; j<nprt; j++) {
      ...
      dist(ndim,&coord[i*ndim],&coord[j*ndim],rij);
      d=magnitude(ndim,rij);
      pot+=1.0/2.0*pow((sin(MIN(d,PI/2.0))),2.0);
    }
  }
  return pot;
}
...
double calckinetic(int nprt,int ndim,double mass,double *veloc)
{
  ...
  double kin=0.0;
  for(i=0; i<nprt; i++) {
    kin += 1.0/2.0*mass*pow((magnitude(ndim,&veloc[i*ndim])),2.0);
  }
  return kin;
}
...

{
  ...
  for(i=0; i<nprt; i++) {
    for(k=0; k<ndim; k++) {
      coord[i*ndim+k] += veloc[i*ndim+k]*dt + 1.0/2.0*pow((dt),2.0)*accel[i*ndim+k];
